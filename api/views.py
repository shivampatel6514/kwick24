    ...
    def post(self, request):
        try:
            ...
            if User.objects.filter(phone=phone).exists():
                return Response({'status': False, 'message': 'Phone number already registered'}, status=status.HTTP_200_OK)
            
            if User.objects.filter(email=data.get('email')).exists():
                return Response({'status': False, 'message': 'Email already registered'}, status=status.HTTP_200_OK)
            # Create user
